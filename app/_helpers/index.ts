...
export const getFeelsLike = (temp1: number, temp2: number): string => {
  let status: string = 'Feels '
  status += temp1 < temp2 ? 'colder' : 'warmer'

  return status

}

export const getPressureValue = (level: number): string => {
  const status: string = Math.round(level) < 1013 ? 'Lower' : 'Higher'

  return `${status} than standard`
}

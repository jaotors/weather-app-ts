import Degree from '@/app/_components/degree'
import Tile from '@/app/_components/tile'

import Sunrise from '@/app/_components/icons/sunrise'
import Sunset from '@/app/_components/icons/sunset'

import type { ForecastType } from '@/app/_types'

import {
  getFeelsLike,
  getHumidityValue,
  getPop,
  getPressureValue,
  getSunTime,
  getVisibilityValue,
  getWindDirection,
} from '@/app/_helpers'

const WEATHER_API_URL = 'https://api.openweathermap.org/data/2.5/forecast'

export default async function Weather({
  params,
}: {
  params: { city: string[] }
}) {
  const [city, lat, lon] = params.city
  const query = `lat=${lat}&lon=${lon}&units=metric&appid=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}`
  const fetchUrl = `${WEATHER_API_URL}?${query}`

  const forecastData: ForecastType = await fetch(fetchUrl)
    .then((response) => response.json())
    .then((data) => {
      console.log(data)
      return {
        ...data.city,
        list: data.list.slice(0, 16),
      }
    })

  const today = forecastData.list[0]

  return (
    <main className='flex justify-center items-center w-full'>
      <section className='w-full md:max-w-[500px] py-4 md:py-4 md:px-10 lg:px-24 h-full lg:h-auto bg-white bg-opacity-20 backdrop-blur-lg rounded drop-shadow-lg'>
        <div className='mx-auto w-[300px]'>
          <section className='text-center'>
            <h2 className='text-2xl font-black'>
              {forecastData.name}{' '}
              <span className='font-thin'>{forecastData.country}</span>
            </h2>
            <h1 className='text-4xl font-extrabold'>
              <Degree temp={Math.round(today.main.temp)} />
            </h1>
            <p className='text-sm'>
              {today.weather[0].main} {today.weather[0].description}
            </p>
            <p className='text-sm'>
              H: <Degree temp={Math.ceil(today.main.temp_max)} /> L:
              <Degree temp={Math.floor(today.main.temp_min)} />
            </p>
          </section>

          <section className='flex overflow-x-scroll mt-4 pb-2 mb-5'>
            {forecastData.list.map((item, i) => {
              const date = i === 0 ? 'Now' : new Date(item.dt * 1000).getHours()
              return (
                <div
                  className='inline-block text-center w-[50px] flex-shrink-0'
                  key={i}
                >
                  <p className='text-sm'>{date}</p>
                  <img
                    src={`http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`}
                    alt={`wetaher-icon-${item.weather[0].description}`}
                  />
                  <p className='text-sm font-bold'>
                    <Degree temp={Math.round(item.main.temp)} />
                  </p>
                </div>
              )
            })}
          </section>

          <section className='flex flex-wrap justify-between text-white'>
            <div className='w-[140px] text-xs font-bold flex flex-col items-center bg-white/20 backdrop-blur-lg rounded drop-shadow-lg py-4 mb-5'>
              <Sunrise />
              <span className='mt-2'>{getSunTime(forecastData.sunrise)}</span>
            </div>
            <div className='w-[140px] text-xs font-bold flex flex-col items-center bg-white/20 backdrop-blur-lg rounded drop-shadow-lg py-4 mb-5'>
              <Sunset />
              <span className='mt-2'>{getSunTime(forecastData.sunset)}</span>
            </div>

            <Tile
              icon='wind'
              title='Wind'
              info={`${Math.round(today.wind.speed)} km/h`}
              description={`${getWindDirection(
                Math.round(today.wind.deg)
              )}, gusts ${today.wind.gust.toFixed(1)} km/h`}
            />
            <Tile
              icon='feels'
              title='Feels like'
              info={<Degree temp={Math.round(today.main.feels_like)} />}
              description={`${getFeelsLike(
                Math.round(today.main.feels_like),
                Math.round(today.main.temp)
              )}`}
            />
            <Tile
              icon='humidity'
              title='Humidity'
              info={`${today.main.humidity}%`}
              description={`${getHumidityValue(today.main.humidity)}`}
            />
            <Tile
              icon='pop'
              title='Precipitation'
              info={`${Math.round(today.pop * 100)}%`}
              description={`${getPop(today.pop)}, clouds at ${
                today.clouds.all
              }%`}
            />
            <Tile
              icon='pressure'
              title='Pressure'
              info={`${today.main.pressure} hPa`}
              description={`${getPressureValue(today.main.pressure)}`}
            />
            <Tile
              icon='visibility'
              title='Visibility'
              info={`${(today.visibility / 1000).toFixed()}`}
              description={`${getVisibilityValue(today.visibility)}`}
            />
          </section>
        </div>
      </section>
    </main>
  )
}
